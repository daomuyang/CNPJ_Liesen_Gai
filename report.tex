...
\begin{itemize}
  ...
  \item SSIM（结构相似性指数，Structural Similarity Index Measure）：从亮度、对比度、结构三个维度衡量两幅图像的结构相似程度，取值范围$[0,1]$，越接近1代表结构保真度越好。采用$11\times 11$ 高斯窗，$K_1=0.01$，$K_2=0.03$，全图计算。SSIM 基于局部滑动窗口计算结构相似性，若强制剔除背景会破坏窗口完整性、引入边界伪影，因此行业内通常保留全图计算；其数值仅用于模型间相对排序，不与 PSNR 做绝对值对比。
\end{itemize}
...
共同骨架：所有 U-Net 系列模型共享编码器$-$解码器骨架，已在第~\ref{sec:protocol} 节详述（$2\times2$ 最大池化下采样、$2\times2$ 转置卷积上采样，卷积块由 Conv$3\times3$$+$BN$+$ReLU $\times 2$ 组成，4 层编码器$+$瓶颈$+$4 层解码器共 5 层深度）。以下分别说明各模型相对于共同骨架的差异。
...
该结果与直觉相反——通常认为感知约束（SSIM）能提供结构先验从而辅助收敛，但在本任务中产生了显著的负面效应。需注意：SSIM 权重 0.2 为常用经验取值，本次消融仅验证该权重下的效果，不排除其他权重下复合损失存在提升空间。
...
就 SE 通道注意力的贡献而言，SE-UNet (32ch) 仅以 $+0.09$M 参数（$+1.1\%$）代价取得 0.05\,dB 的微弱领先，说明 SE 注意力在 T1 去噪中存在可检测但幅度有限的正面贡献。

UNet++ Lite 的参数效率值得关注：以 2.00M 最少参数量（仅为 U-Net 32ch 的 25.7\%）达到 2.62\,dB 的次优性能，其嵌套密集跳连设计提供了多条短梯度路径，在 1500 步这样的短步长下收敛速度显著快于标准 U-Net，使其在这一训练阶段表现占优。

对比 U-Net 32ch 与 64ch，4 倍参数量差距仅换来 $-0.01$\,dB 的性能差异（统计不可分辨），说明在 1500 步预算下 32 个基础通道已提供足够的表示容量。

RED-CNN 性能较弱，可能因为其在 5 层编码器中使用 $3\times 3$ 卷积堆栈，感受野增长线性而 U-Net 系列通过池化实现指数级感受野扩张，在全局噪声分布建模上处于劣势。
...
本报告所有实验采用等步数比较——回答"相同训练预算下最优"的问题，而非各模型充分收敛后的理论上限。固定 epoch 会使大模型消耗更多算力，不适合资源受限场景。在本次总 GPU 时间约 2.3 小时的预算下，资源投入应视为建模成本的一部分；需要说明的是，U-Net 64ch 可能存在在 1500 步下训练不充分而性能被低估的情形，若获得更多步数有可能释放其容量优势，但在给定的固定预算约束下这一成本无法被忽略。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l c c c c c c}
    ...
    SE-UNet 32ch & 2.43 & 0.90 & 2.43 & 1.92 & 0.89 & 1.77 \\
    \bottomrule
  \end{tabular}
\end{table}

图~\ref{fig:stability} 和表~\ref{tab:stability} 共同揭示了三类跨种子行为模式（注意 Stage C 与 Stage B 的训练步数不同，均值绝对值不直接可比，稳定性分析聚焦于种子间离散度而非均值本身）：

第一类为高稳定性组，包括 UNet++ Lite 和 U-Net 32ch，两者跨种子标准差均 $\le 0.09$\,dB，$2\sigma$ 分别仅 0.17\,dB 和 0.19\,dB，图~\ref{fig:stability}b 中六条训练曲线紧密聚合，可复现性优秀。

第二类为中不稳定组——U-Net 64ch，跨种子 $2\sigma=0.82$\,dB，Seed 42 仅获 1.46\,dB 与 seed 123 的 2.26\,dB 相差 0.80\,dB。核心原因是 1000 步预算对 31.04M 参数的大模型严重不足，不同随机种子停留在差异巨大的鞍点或局部极小值。

第三类为训练失效组——SE-UNet (32ch)，seed 123 下 val gain 仅 0.90\,dB，不到其他两个种子（2.43、2.43\,dB）的 40\%。图~\ref{fig:stability}b 中 seed 123 轨迹在约 400 步后停滞于 1.0\,dB 以下，训练损失同步在高位停滞、无下降趋势，与梯度消失的特征吻合。推测机制为：SE 模块中 Sigmoid 门控在随机初始化时，若 GAP 压缩向量落入 Sigmoid 饱和区，$a(1-a) \approx 0$ 导致门控梯度消失，通道注意力退化且额外引入参数噪声，扰动跳跃连接梯度路径。该现象仅在 1000 步中期训练中观测到；5000 步全量训练中 seed 42 未出现失效，不排除更长训练步数下部分失效种子可自行恢复。本次 3 种子观测中出现 1 例训练失效，提示该架构存在初始化敏感的收敛失败风险；实际使用时建议增加跨种子验证以规避此风险。
...
SE-UNet 稳居数值最优但优势微小：test gain 2.77\,dB，相比 U-Net 32ch（2.68\,dB）仅高出 0.09\,dB，在 Tier-1 候选间统计不可分辨。SE 注意力在 5000 步充足训练下带来了微弱但全维度一致的提升。
...
\begin{itemize}
  \item SE-UNet (32ch) 在 seed 42 下精度最优（48.23\,dB），但 Stage C 中期观测到 seed 123 训练失效（$2\sigma=1.77$\,dB），存在初始化敏感的收敛失败风险。
  \item U-Net 64ch 以 4 倍参数量仅换得 $+0.05$\,dB 精度改善，参数效率极低，跨种子波动亦非最优（$2\sigma=0.82$\,dB）。
  \item UNet++ Lite 全量数据下出现容量$-$数据错配——验证增益从 step 250 的 2.29\,dB 崩溃至训练终点 0.3--0.5\,dB，2M 参数容量无法适配 480 例数据规模。
  \item U-Net 32ch 在所有维度均衡——精度仅落后 0.09\,dB（统计不可分辨），中期训练跨种子 $2\sigma=0.19$\,dB（可复现性优异），7.76M 参数适中。
\end{itemize}

综合以上证据，本报告推荐 U-Net 32ch 为最终方案。选型依据是跨种子稳定性与可复现性——精度维度在 Tier-1 候选间统计不可分辨，而稳定性维度提供了关键的区分信号。跨种子稳定性直接决定实验的可复现性与工程落地成本：U-Net 32ch 的低波动（$2\sigma=0.19$\,dB）意味着单次训练即可获得稳定可靠的结果，无需多次重复筛选最优种子，在资源受限的科研场景与批量部署中具有更高的实用价值。稳定性结论基于 Stage C 中期训练（1000 步）观测，全量收敛后的长期稳定性仍待进一步验证。
...
最佳增益病例（图~\ref{fig:triplet_best}）：该病例（3D gain $\approx$ 3.90\,dB）初始含噪图像中灰白质边界和皮层沟回处存在明显的结构模糊。去噪后这些高对比度解剖边界得到清晰恢复，局部放大细节中去噪输出与干净参考的差异肉眼难辨。绝对误差图显示误差几乎全部集中在解剖边界和脑沟区域；梯度$-$误差联合图进一步揭示：这些边界区域的梯度模较大（品红高亮），而平滑均匀区（白质内部）梯度极小且对应误差接近零——模型在解剖结构变化剧烈处仍有可观测的残留误差，但在均匀组织内部实现了近乎完美的恢复。

最低增益病例（图~\ref{fig:triplet_worst}）：该病例（3D gain $\approx$ 0.77\,dB）的初始含噪图像 PSNR 极高（$\approx$ 50.31\,dB），含噪图在视觉上与干净参考几乎无异。这不是模型失效，而是度量天花板效应：当噪声极小时，PSNR gain 的计算空间被压缩。该病例的存在也说明数据集内噪声水平的病例间差异显著（标准差比例范围 0.013--0.042）。

中位病例（图~\ref{fig:triplet_median}）：误差模式同样集中于灰白质交界和皮层沟回处，深部白质核心和脑室附近误差极低——与 T1 的组织信号分布一致。所有去噪输出均未引入可见的结构伪影，残差学习在纹理保真度上具有明显优势。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.80\textwidth]{signal_bin_bar.png}
...
综合以上分析，本工作虽未针对 Rician 噪声进行专项校正，但通过 ME 指标和信号分箱进行了系统追踪，三条证据链共同确认了 Rician 信号依赖性的真实存在且被模型有效学习：(1) 分箱 gain 梯度 6.72 $\to$ 2.36 $\to$ 1.58\,dB 与 Rician 方差$-$信号关系吻合；(2) 含噪 ME 从 $+3.5\times 10^{-3}$ 单调降至 $+1.2\times 10^{-3}$；(3) 去噪后 ME 全局收束至 $-3.1\times 10^{-4}$，表明通用 L1 残差学习已能有效压制 Rician 正偏差。需注意 ME 仅反映全局均值偏移，无法区分组织类型或空间位置的偏差异质性。

\subsection{参数效率权衡}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.80\textwidth]{param_efficiency.png}
  \caption{参数—性能权衡（Stage B，1500 步）}
  \label{fig:param}
\end{figure}

图~\ref{fig:param} 将 Stage B 中五种架构的参数—性能权衡可视化，揭示了三条不同的设计路径。
...
\subsection{局限与展望}
\begin{enumerate}
  \item 2D 切片逐层处理未利用 z 轴层间连续性，3D 卷积或 2.5D 多切片输入预期可获额外增益~\cite{Zhao2021MC3DCNN}。
  \item 结论仅基于 T1 加权与单次数据划分（80/10/10, seed 42），泛化性待交叉验证确认。
  \item Stage C 仅覆盖 3 个种子，更大规模（$\ge 10$）的种子扫描将提供更可靠的稳定性估计。
  \item 跨种子稳定性结论基于 1000 步中期观测，全量 5000 步下的长期波动需进一步验证。
  \item 信号分箱使用简单的三等分，组织类别特异性分析（如经 FSL FAST 分割）可提供更精细的见解。
\end{enumerate}
